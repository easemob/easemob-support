...
void initSDK() async {
  ...
  MethodChannel _channel = const MethodChannel('flutter_iOS');
  final result = await _channel.invokeMethod('getDeviceToken');
  Map map = result as LinkedHashMap<Object?, Object?>;
  print("deviceToken: ${map["deviceToken"]}");
  final deviceToken = map["deviceToken"];
  if (deviceToken != ""){
    await EMClient.getInstance.pushManager.updateAPNsDeviceToken(deviceToken);
  }
  print("has init");
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void _incrementCounter() {
    ...
    //   _counter++;
    // });
    EMClient.getInstance.login("wf1", "1");
  }
